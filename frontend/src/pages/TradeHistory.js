...
export default function TradeHistory({ embedded = false }) {
  ...
  return (
    <div className="space-y-6" data-testid="trades-page">
      {!embedded && (
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-50">Trade History</h1>
      ...
      <Card>
        ...
        <div className="flex flex-col md:flex-row gap-3 px-5 py-4">
          <Input placeholder="Ticker" value={ticker} data-testid="filter-ticker"
            onChange={(e) => setTicker(e.target.value.toUpperCase())} className="md:w-40 font-mono uppercase" />
          <div className="flex flex-col">
            <label className="text-[10px] uppercase text-zinc-500 mb-0.5">From</label>
            <Input type="date" value={start} data-testid="filter-start" onChange={(e) => setStart(e.target.value)} className="md:w-44" />
          </div>
          <div className="flex flex-col">
            <label className="text-[10px] uppercase text-zinc-500 mb-0.5">To</label>
            <Input type="date" value={end} data-testid="filter-end" onChange={(e) => setEnd(e.target.value)} className="md:w-44" />
          </div>
          <div className="flex items-end gap-2">
            <Button onClick={load} data-testid="apply-filters-btn"><Filter size={15} /> Apply</Button>
            <Button variant="outline" onClick={clearFilters} data-testid="clear-filters-btn"><X size={15} /> Clear</Button>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        {loading ? (
          ...
          <EmptyState title="No trades found" hint="Trades appear here once the strategy places orders." icon={Filter} />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm" data-testid="trades-table">
              ...
              <tbody className="font-mono">
                {trades.map((t) => (
                  <tr key={t.id} className="border-b border-zinc-800/60 hover:bg-zinc-800/40" data-testid={`trade-row-${t.id}`}>
                    <td className="px-5 py-2.5 text-zinc-500 whitespace-nowrap">{fmtDate(t.timestamp)}</td>
                    <td className="px-3 py-2.5 font-semibold text-zinc-100">{t.ticker}</td>
                    <td className="px-3 py-2.5"><Badge tone={t.side === "buy" ? "success" : "danger"}>{t.side}</Badge></td>
                    <td className="px-3 py-2.5 text-right tabular text-zinc-300">{fmtNum(t.quantity)}</td>
                    <td className="px-3 py-2.5 text-right tabular text-zinc-300">{fmtUSD(t.price)}</td>
                    <td className={`px-3 py-2.5 text-right tabular ${t.realized_pnl == null ? "text-zinc-600" : t.realized_pnl >= 0 ? "text-profit" : "text-loss"}`}>
                      {t.realized_pnl == null ? "—" : fmtUSD(t.realized_pnl)}
                    </td>
                    <td className="px-3 py-2.5 text-zinc-400 font-sans text-xs max-w-md">{t.trigger_reason}</td>
                    <td className="px-5 py-2.5 text-right">
                      <button className="text-klein text-xs hover:underline" onClick={() => setDetail(t)} data-testid={`view-params-${t.id}`}>
                        view
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setDetail(null)}>
          <Card className="w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <CardHeader
              ...
              right={<button onClick={() => setDetail(null)} data-testid="close-detail" className="text-zinc-400 hover:text-zinc-100"><X size={18} /></button>}
            />
            <div className="px-5 py-4 space-y-3">
              <div className="text-xs text-zinc-500">{fmtDate(detail.timestamp)} · order {detail.order_id || "—"}</div>
              <div className="text-sm text-zinc-300">{detail.trigger_reason}</div>
              <pre className="bg-zinc-950 border border-zinc-800 text-zinc-300 rounded-lg p-3 text-xs font-mono overflow-auto" data-testid="params-snapshot-json">
